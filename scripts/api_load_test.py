...
def run_concurrent_tests(num_concurrent: int, num_requests: int) -> List[Tuple[bool, float]]:
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=num_concurrent) as executor:
        future_to_request = {executor.submit(make_prediction_request): i for i in range(num_requests)}
        ...
        for future in concurrent.futures.as_completed(future_to_request):
            ...
            try:
                ...
                print(f"Request {request_num+1}/{num_requests}: {'Success' if success else 'Failed'} in {time_taken:.4f}s")
            except Exception as e:
