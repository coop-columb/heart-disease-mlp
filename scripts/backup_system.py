...
def upload_to_azure(archive_path: Path, credentials: Dict) -> bool:
    ...
    try:
        ...
        logger.error("azure-storage-blob not installed. Run 'pip install azure-storage-blob' to enable Azure uploads.")
        return False
    except Exception as e:
...
def upload_to_gcp(archive_path: Path, credentials: Dict) -> bool:
    ...
    try:
        ...
        logger.error("google-cloud-storage not installed. Run 'pip install google-cloud-storage' to enable GCP uploads.")
        return False
    except Exception as e:
...
def download_from_azure(timestamp: str, download_path: Path, credentials: Dict) -> Optional[Path]:
    ...
    try:
        ...
        logger.error("azure-storage-blob not installed. Run 'pip install azure-storage-blob' to enable Azure downloads.")
        return None
    except Exception as e:
...
def download_from_gcp(timestamp: str, download_path: Path, credentials: Dict) -> Optional[Path]:
    ...
    try:
        ...
        logger.error("google-cloud-storage not installed. Run 'pip install google-cloud-storage' to enable GCP downloads.")
        return None
    except Exception as e:
...
def list_backups(cloud: bool = False, storage: str = None) -> List[Dict]:
    ...
    for idx, backup in enumerate(backups):
        ...
        logger.info(f"{idx+1}. Timestamp: {timestamp} | Created: {creation_time} | Files: {file_count} | Size: {size_str}")
...
def list_s3_backups(credentials: Dict) -> None:
    ...
    try:
        ...
        response = s3_client.list_objects_v2(
            Bucket=bucket_name,
            Prefix="heart_disease_mlp/backups/"
        )
        ...
        for idx, obj in enumerate(response["Contents"]):
            ...
            logger.info(f"{idx+1}. Timestamp: {timestamp} | Modified: {last_modified} | Size: {size_str}")
...
def list_azure_backups(credentials: Dict) -> None:
    ...
    try:
        ...
        logger.error("azure-storage-blob not installed. Run 'pip install azure-storage-blob' to enable Azure operations.")
    except Exception as e:
...
def list_gcp_backups(credentials: Dict) -> None:
    ...
    try:
        ...
        logger.error("google-cloud-storage not installed. Run 'pip install google-cloud-storage' to enable GCP operations.")
    except Exception as e:
...
def prune_backups(keep: int = 5, cloud: bool = False, storage: str = None) -> bool:
    ...
    if len(backups) > keep:
        ...
        logger.info(f"Keeping {len(backups_to_keep)} most recent backups, deleting {len(backups_to_delete)} older backups.")
        ...
        logger.info(f"Only {len(backups)} backups exist, which is less than or equal to the {keep} backups to keep. No pruning needed.")
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Backup and recovery system for Heart Disease Prediction project")
    subparsers = parser.add_subparsers(dest="command", help="Command to execute")
    
    # Backup command
    backup_parser = subparsers.add_parser("backup", help="Create a new backup")
    backup_parser.add_argument("--cloud", action="store_true", help="Upload backup to cloud storage")
    backup_parser.add_argument("--storage", choices=["s3", "azure", "gcp"], help="Cloud storage provider")
    
    # Restore command
    restore_parser = subparsers.add_parser("restore", help="Restore from a backup")
    restore_parser.add_argument("--timestamp", help="Backup timestamp to restore (default: latest)")
    restore_parser.add_argument("--cloud", action="store_true", help="Download backup from cloud storage")
    restore_parser.add_argument("--storage", choices=["s3", "azure", "gcp"], help="Cloud storage provider")
    
    # List command
    list_parser = subparsers.add_parser("list", help="List available backups")
    list_parser.add_argument("--cloud", action="store_true", help="List cloud backups")
    list_parser.add_argument("--storage", choices=["s3", "azure", "gcp"], help="Cloud storage provider")
    
    # Prune command
    prune_parser = subparsers.add_parser("prune", help="Prune old backups")
    prune_parser.add_argument("--keep", type=int, default=5, help="Number of most recent backups to keep")
    prune_parser.add_argument("--cloud", action="store_true", help="Prune cloud backups")
    prune_parser.add_argument("--storage", choices=["s3", "azure", "gcp"], help="Cloud storage provider")
