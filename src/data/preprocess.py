...
def binarize_target(df: pd.DataFrame, target_col: str = 'target') -> pd.DataFrame:
    ...
    logger.info(f"Target distribution after binarization: {df[target_col].value_counts().to_dict()}")
    return df
        ...
        for col in categorical_cols:
            if df[col].isnull().sum() > 0:
                if categorical_strategy == 'most_frequent':
                    ...
                    raise ValueError(f"Unknown categorical imputation strategy: {categorical_strategy}")
    else:
...
def split_data(X: pd.DataFrame, y: pd.Series, test_size: float = 0.2, 
    ...
    logger.info(f"Data split complete. Shapes: X_train={X_train.shape}, X_val={X_val.shape}, X_test={X_test.shape}")
